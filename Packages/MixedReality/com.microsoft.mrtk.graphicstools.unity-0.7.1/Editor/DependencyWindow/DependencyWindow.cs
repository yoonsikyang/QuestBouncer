...
{
    /// <summary>
    /// The Dependency Window displays how assets reference and depend on each other. Dependencies are calculated by parsing GUIDs within 
    /// project YAML files (note, script to script dependencies are not considered). 
    /// 
    /// The window displays a list of assets that the currently selected asset depends on, and a hierarchical list of assets that depend 
    /// on it. If nothing depends on the currently selected asset, you can consider deleting it from your project (note that some assets 
    /// are loaded programmatically via APIs like Shader.Find() and may not be caught by the dependency tracker). The window can also 
    /// display just a list of all assets which are not referenced by any other assets and could be considered for deletion.
    /// </summary>
    public class DependencyWindow : EditorWindow
    {
        ...
        [MenuItem("Window/Graphics Tools/Dependency Window")]
        ...
        private void OnGUI()
        {
            ...
            else if (selectedToolbarIndex == 1)
            {
                ...
                excludePackageAssets = EditorGUILayout.Toggle("Exclude Package Assets", excludePackageAssets);
                ...
                EditorGUILayout.LabelField(new GUIContent("Some asset types are always excluded", EditorGUIUtility.IconContent("_Help").image, tooltip));
            ...
            EditorGUILayout.BeginVertical("Box");
            {
                ...
                scrollPosition = EditorGUILayout.BeginScrollView(scrollPosition);
                {
                    foreach (var kvp in dependencyGraph)
                    {
                        ...
                        if (excludePackageAssets && IsInPackages(node))
                        {
                            continue;
                        }
        ...
        private static bool IsInPackages(DependencyGraphNode node)
        {
            if (!string.IsNullOrEmpty(node.path))
            {
                return node.path.StartsWith("Packages");
            }
