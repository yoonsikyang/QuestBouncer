...
#pragma warning restore 0618
                ...
                case AlignmentType.AlignWithHeadHeight:
                    if (!TryGetMainCamera(out Camera headHeightCamera))
                    {
                        contentInitialized = true;
                        return;
                    }
                    ...
                    contentPosition.x = containerObject.position.x;
                    contentPosition.y = containerObject.position.y + headHeightCamera.transform.position.y;
                    contentPosition.z = containerObject.position.z;
        ...
        {
            if (!TryGetMainCamera(out Camera headPoseCamera))
            {
                return;
            }

            contentPosition.x = headPoseCamera.transform.localPosition.x;
            contentPosition.y = headPoseCamera.transform.localPosition.y;
            contentPosition.z = headPoseCamera.transform.localPosition.z;

            contentOrientation.y = headPoseCamera.transform.rotation.y;
            contentOrientation.w = headPoseCamera.transform.rotation.w;
        ...
        {
            mainCamera = CameraCache.Main != null ? CameraCache.Main : Camera.main;
            if (mainCamera != null)
            {
                return true;
            }
