...
namespace Microsoft.MixedReality.Toolkit.UI.Editor
{
    ...
    public class ThemeInspector : UnityEditor.Editor
    {
        ...
        private const string CompatTag = "[MRTK Compat] ThemeInspector";
                    ...
                    if (show)
                    {
                        ...
                        using (new EditorGUI.IndentLevelScope())
                        {
                            ...
                            using (new EditorGUILayout.HorizontalScope())
                            {
                                ...
                                {
                                    if (!TryRecoverThemeDefinitionAssemblyQualifiedName(className.stringValue, themeTypes.ToArray(), out var recoveredAqn))
                                    {
                                        Debug.LogError($"{CompatTag} Could not safely recover AssemblyQualifiedName for ThemeDefinition '{className.stringValue}'. Re-select Theme Runtime in inspector.");
                                    }
                                    else
                                    {
                                        assemblyQualifiedName.stringValue = recoveredAqn;
                                        Debug.LogWarning($"{CompatTag} Restored missing AssemblyQualifiedName for ThemeDefinition '{className.stringValue}' using legacy migration.");
                                    }
                                }
        ...
        // Legacy migration helper for serialized ThemeDefinition assets missing AssemblyQualifiedName.
        // Recovery is only applied when there is exactly one class-name match; ambiguous or missing
        // matches are intentionally not auto-fixed and must be manually reselected in the inspector.
        private bool TryRecoverThemeDefinitionAssemblyQualifiedName(string className, Type[] themeTypes, out string recoveredAssemblyQualifiedName)
        {
            recoveredAssemblyQualifiedName = null;

            if (string.IsNullOrEmpty(className) || themeTypes == null || themeTypes.Length == 0)
            {
                return false;
            }

            var matches = themeTypes.Where(t => t != null && t.Name == className).ToArray();
            if (matches.Length == 1)
            {
                recoveredAssemblyQualifiedName = matches[0].AssemblyQualifiedName;
                return true;
            }

            if (matches.Length == 0)
            {
                Debug.LogError($"{CompatTag} Could not find ThemeBase type for ClassName '{className}'.");
            }
            else
            {
                Debug.LogError($"{CompatTag} Ambiguous legacy ThemeDefinition migration for ClassName '{className}'. Multiple matches found.");
            }
        ...
        private static void RenderShaderProperties(SerializedProperty themeDefinition)
        {
            ...
            for (int i = 0; i < stateProperties.arraySize; i++)
            {
                ...
                if (ThemeStateProperty.IsShaderPropertyType((ThemePropertyTypes)type.intValue))
                {
                    ...
                    // Compatibility shim for serialized theme data that predates current shader field schema.
                    // If schema fields are missing, apply deterministic migration once and continue with defaults.
        ...
        /// <summary>
        /// Compatibility utility that upgrades legacy shader fields to current schema with deterministic defaults.
        /// If deprecated fields are absent, this keeps a known-safe shader default instead of guessing.
        /// </summary>
