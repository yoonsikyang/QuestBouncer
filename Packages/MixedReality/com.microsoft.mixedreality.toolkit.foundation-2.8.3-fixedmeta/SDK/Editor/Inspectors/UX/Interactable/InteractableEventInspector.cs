...
namespace Microsoft.MixedReality.Toolkit.UI
{
    public static class InteractableEventInspector
    {
        ...
        private const string CompatTag = "[MRTK Compat] InteractableEventInspector";
            ...
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    using (new EditorGUI.PropertyScope(position, SelectEventLabel, className))
                    {
                        ...
                        {
                            if (!TryRecoverAssemblyQualifiedName(className.stringValue, receiverTypes.ToArray(), out var recoveredAqn))
                            {
                                Debug.LogError($"{CompatTag} Could not safely recover AssemblyQualifiedName for ClassName '{className.stringValue}'. Re-select the event receiver in inspector.");
                            }
                            else
                            {
                                assemblyQualifiedName.stringValue = recoveredAqn;
                                Debug.LogWarning($"{CompatTag} Restored missing AssemblyQualifiedName for ClassName '{className.stringValue}' using legacy migration. Verify serialized data if this project uses custom builds.");
                            }
                        }
        ...
        // Legacy migration helper for serialized Interactable events where ClassName was captured but AssemblyQualifiedName was not saved.
        // We only allow recovery when there is exactly one class-name match; ambiguous or unresolved assets are not auto-fixed.
        private static bool TryRecoverAssemblyQualifiedName(string className, Type[] receiverTypes, out string recoveredAssemblyQualifiedName)
        {
            recoveredAssemblyQualifiedName = null;

            if (string.IsNullOrEmpty(className) || receiverTypes == null)
            {
                return false;
            }

            var matches = receiverTypes.Where(t => t != null && t.Name == className).ToArray();
            if (matches.Length == 1)
            {
            ...
            if (matches.Length == 0)
            {
                Debug.LogError($"{CompatTag} No matching ReceiverBase type found for legacy ClassName '{className}'.");
            }
            else
            {
                Debug.LogError($"{CompatTag} Ambiguous legacy migration for ClassName '{className}'. Multiple ReceiverBase types found.");
            }
